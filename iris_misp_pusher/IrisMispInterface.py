...
class IrisMispInterface(IrisModuleInterface):
    ...
    def hooks_handler(self, hook_name, hook_ui_name, data):
        ...
        # Note: To export MISP data to files (/var/www/html/misp-*.txt), 
        # run update_to_misp.py script on the host machine (not in container)
        if newly_added_count > 0:
            self.log.info("Tip: Run update_to_misp.py on host to export MISP data to files")
